...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    let byte_coordinates: Vec<(u32, u32)> = input.split_whitespace()
        .map(|str| {
    ...
    let kilo_coords = byte_coordinates[0..kilo].to_vec();
...
fn bfs_search(grid: &Vec<Vec<bool>>, start_coord: (usize, usize), max: i32) -> Option<HashMap<(usize, usize), (usize, usize)>> {
    ...
    while !to_search.is_empty() {
        let coord = to_search.pop_front().unwrap();
         for dir in dirs {
            ...
            if !(searched.contains_key(&(x as usize,y as usize)) || x < 0 || y < 0 || x >= max || y >= max) && !grid[y as usize][x as usize ] {
