...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    result += re_xmas
        .captures_iter(&char_matrix_to_string_matrix(&char_matrix))
        .count() as u32;
    result += re_xmas
        .captures_iter(&char_matrix_to_string_matrix(&char_matrix_t))
        .count() as u32;
    result += re_xmas
        .captures_iter(&char_matrix_to_string_matrix(&diag_matrix))
        .count() as u32;
    result += re_xmas
        .captures_iter(&char_matrix_to_string_matrix(&diag_matrix_t))
        .count() as u32;
    result += re_samx
        .captures_iter(&char_matrix_to_string_matrix(&char_matrix))
        .count() as u32;
    ...
    for i in 0..char_matrix.len() {
        for j in 0..char_matrix[i].len() {
            if char_matrix[i][j] == 'A' {
                ...
                result += strings
                    .iter()
                    .fold(true, |acc, s| acc && re_mas_sam.is_match(s))
                    as u32;
            }
        }
    }
